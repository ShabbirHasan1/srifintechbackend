...
from . import classes , chartjs_classes
import pandas as pd
...
class Open_Interst_Chart_API_View(APIView):
    ...
    def post(self , request):
        ...
        ##################Input parameters #####################
        oi = classes.OIAnalysis()
        kf = classes.KiteFunctions()
        po= classes.PostgreSQLOperations()
        ...
        openinterest_newbardata = chartjs_classes.oichange_newbardata
        openinterest_bargraph = chartjs_classes.oichange_bargraph
...
class MaxPain_History_Chart_API_View(APIView):
    ...
    def post(self , request):
        ...
        query_op = db.execute("select * from maxpain_history where date(expiry_date)='{}' and  date(date) >='{}' and ticker='NIFTY' order by date desc".format(expiry_date , from_date)).fetchall()
        ...
        if ltd == query_op[0]['date']:
            ...
            data_temp = get_data_db(3)
            data += data_temp

        main_df = pd.DataFrame( data =data , columns=['date','maxpain_value','price'])

        print(main_df)

        ####################### chartjs ########################
        NewChart = chartjs_classes.maxpain_linegraph(label_ticker=ticker,scale_label_str="Maxpain")()
